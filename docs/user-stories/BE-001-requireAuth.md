...
- [x] Estrae il token dall'header `Authorization: Bearer <token>`
- [x] Verifica firma JWT con la chiave segreta del server
- [x] Decodifica il payload e lo aggiunge a `req.user`
- [x] Restituisce `401 Unauthorized` se l'header è assente o malformato
- [x] Restituisce `401 Unauthorized` se il token è scaduto o firma non valida
- [x] Non blocca gli endpoint pubblici (es. `GET /matches`, `GET /matches/:id`)
